...
class RetroCrossAttention(nn.Module):
    ...
    def forward(self, x, ca_hidden_states, ca_attention_mask):
        ...
        # print("Input dtypes: x: {}\tca_hidden_states: {}\tca_attention_mask: {}".format(x.dtype, ca_hidden_states.dtype, ca_attention_mask.dtype))
        # print("Input shapes: x: {}\tca_hidden_states: {}\tca_attention_mask: {}".format(x.shape, ca_hidden_states.shape, ca_attention_mask.shape))
...
class RetroChunkedCrossAttention(nn.Module):
    ...
    def forward(self, x, cca_hidden_states, cca_attention_mask, retrieval_interval: int):
        ...
        ### Wenqi: here, split the decoder's states based on retrieval interval, rather than chunk_size
        # Split up input into attending chunks
        pad_len = retrieval_interval-1 # Base padding, truncate x before self.chunk_size-1, because the second decoder chunk relies on the first retrieved chunk
        # pad_len = self.chunk_size-1 # Base padding, truncate x before self.chunk_size-1, because the second decoder chunk relies on the first retrieved chunk
        if (x.shape[1] - (self.chunk_size - 1) + pad_len) % retrieval_interval != 0: 
            pad_len += retrieval_interval - (x.shape[1] - self.chunk_size) % retrieval_interval
        ...
        assert attending_chunks.shape[1] % retrieval_interval == 0
        attending_chunks = rearrange(attending_chunks, "b (l m) d -> b l m d", m=retrieval_interval)
        ...
        # Wenqi: here, everything whose query window requires the latest 64-token should be masked out
        # Wenqi: update, no need to do it for evaluation! In evaluation, we just need to make sure that the number of output chunks and the neighbour chunks can be matched one-by-one, we already
        #   removed the first chunk of output to make a match, so we don't need to mask out anything here
        # completely_masked_chunks = torch.tensor([[True] * cca_hidden_states.shape[1]] * cca_hidden_states.shape[0], device=q.device)
        # num_complete_chunks = int((x.shape[1] - self.chunk_size) / retrieval_interval + 1)
        # completely_masked_chunks = torch.tensor([[False] * num_complete_chunks + [True] * (cca_hidden_states.shape[1] - num_complete_chunks)] * cca_hidden_states.shape[0], device=q.device)
        # mask_last_chunk_num = int(self.chunk_size / retrieval_interval if self.chunk_size % retrieval_interval == 0 else self.chunk_size / retrieval_interval + 1)
        # completely_masked_chunks = torch.tensor([[False] * (cca_hidden_states.shape[1] - mask_last_chunk_num) + [True] * mask_last_chunk_num] * cca_hidden_states.shape[0], device=q.device)

        # original:
        completely_masked_chunks = torch.all(rearrange(torch.logical_not(cca_attention_mask), "b l k r -> b l (k r)"), dim=-1)

        # # allow all:
        # completely_masked_chunks = torch.tensor([[False] * cca_hidden_states.shape[1]] * cca_hidden_states.shape[0], device=q.device)
        ...
        if pad_len > 0:
            ...
            output = F.pad(output[:, :, :], (0, 0, self.chunk_size-1, 0), "constant", 0)
...
class RetroEncoder(nn.Module):
    ...
    def forward(
        ...
        # print("RetroEncoder:")
        # print("Input dtypes: input_ids: {}\tattention_mask: {}\tca_hidden_states: {}\tca_attention_mask: {}".format(input_ids.dtype, attention_mask.dtype, ca_hidden_states.dtype, ca_attention_mask.dtype))
        # print("Input shapes: input_ids: {}\tattention_mask: {}\tca_hidden_states: {}\tca_attention_mask: {}".format(input_ids.shape, attention_mask.shape, ca_hidden_states.shape, ca_attention_mask.shape))
...
class RetroModel(nn.Module):
    ...
    def set_retrieval_interval(self, retrieval_interval: int):
    ...
    def forward(
        ...
        input_ids,
        neighbour_ids,
        input_attention_mask, # decoder self-attention mask (casual)
        neighbour_attention_mask, # encoder neighbour self-attention mask (across chunks?)
        ca_attention_mask, # encoder neighbour-to-decoder cross-attention mask
        return_all_hidden_states=False,
        ...
        if input_attention_mask is None:
            input_attention_mask = (input_ids != self.config.pad_token_idx).type(torch.float16).to(input_ids.device)
        if neighbour_attention_mask is None:
            neighbour_attention_mask = (neighbour_ids != self.config.pad_token_idx).type(torch.float16).to(input_ids.device)
        if ca_attention_mask is None:
            ca_attention_mask = input_attention_mask
        # # print("neighbour_ids:", neighbour_ids)
        neighbour_ids = neighbour_ids.to(input_ids.device)
        input_attention_mask = input_attention_mask.to(input_ids.device)
        neighbour_attention_mask = neighbour_attention_mask.to(input_ids.device)
        ca_attention_mask = ca_attention_mask.to(input_ids.device)
        ...
        for dec_layer_idx, (sa, cca, ffn) in enumerate(self.decoder):
            ...
            if dec_layer_idx == min(self.config.dec_cca_layers or [-1]):
                ...
                ca_hidden_states = x
                if x.shape[1] < self.config.chunk_size:
                    ca_hidden_states = F.pad(ca_hidden_states, (0, 0, 0, self.config.chunk_size - x.shape[1]), mode="constant", value=0.0)
                    ca_attention_mask = F.pad(ca_attention_mask, (0, self.config.chunk_size - x.shape[1]), mode="constant", value=0)
                if (ca_hidden_states.shape[1] - self.config.chunk_size) % self.retrieval_interval != 0:  # Pad to full chunk length, to be able to reshape
                    # print(ca_hidden_states.shape[1], self.retrieval_interval)
                    ca_hidden_states = F.pad(ca_hidden_states, (0, 0, 0, self.retrieval_interval - (ca_hidden_states.shape[1] - self.config.chunk_size) % self.retrieval_interval), mode="constant", value=0.0)
                    ca_attention_mask = F.pad(ca_attention_mask, (0, self.retrieval_interval - (ca_hidden_states.shape[1] - self.config.chunk_size) % self.retrieval_interval), mode="constant", value=0)
                assert (ca_hidden_states.shape[1] - self.config.chunk_size) % self.retrieval_interval == 0

                ### Wenqi: here, need to replicate some dimensions of ca_hidden_states
                # extend the states and masks, such that 
                num_chunks = int(math.ceil((ca_hidden_states.shape[1] - self.config.chunk_size) / self.retrieval_interval) + 1)
                # print(num_chunks)
                bs, out_len, hidden_size = ca_hidden_states.shape
                ca_hidden_states_duplicated = torch.zeros((bs, num_chunks * self.config.chunk_size, hidden_size), dtype=ca_hidden_states.dtype, device=ca_hidden_states.device)
                ca_attention_mask_duplicated = torch.zeros((bs, num_chunks * self.config.chunk_size), dtype=ca_attention_mask.dtype, device=ca_attention_mask.device)
                # copy from original ca_hidden_states
                for n in range(num_chunks):
                    ca_hidden_states_duplicated[:, n * self.config.chunk_size : (n+1) * self.config.chunk_size, :] = ca_hidden_states[:, n * self.retrieval_interval : n * self.retrieval_interval + self.config.chunk_size, :]
                    ca_attention_mask_duplicated[:, n * self.config.chunk_size : (n+1) * self.config.chunk_size] = ca_attention_mask[:, n * self.retrieval_interval : n * self.retrieval_interval + self.config.chunk_size]

                ca_hidden_states = rearrange(ca_hidden_states_duplicated, "b (l m) d -> b l m d", m=self.config.chunk_size)
                ca_attention_mask = rearrange(ca_attention_mask_duplicated, "b (l m) -> b l m", m=self.config.chunk_size)
            ...
            ### Wenqi: here, need to rearrange x's shape to match the retrieval interval in the cca forward function
            if cca is not None:
                x, _ = cca(x, neighbour_hidden_states, neighbour_attention_mask, self.retrieval_interval)
...
class RetroModelLMHead(nn.Module, GenerationMixin):
    ...
    def __init__(self, config: RetroConfig, retriever: Optional[Retriever]=None, device="cpu", **kwargs):
        ...
        self.set_retrieval_interval(self.config.chunk_size)

    def set_retrieval_interval(self, retrieval_interval: int):
        self.base.set_retrieval_interval(retrieval_interval)
    ...
    def prepare_inputs_for_generation(self, input_ids: torch.LongTensor, **kwargs) -> Dict[str, Any]:
        ...
        num_chunks = int(np.ceil((input_ids.shape[1] - self.config.chunk_size) / self.retrieval_interval)) + 1
        num_complete_chunks = int(np.floor((input_ids.shape[1] - self.config.chunk_size) / self.retrieval_interval)) + 1
        ...
        if neighbour_ids.shape[1] < num_chunks:
            ...
            for chunk_idx in chunks_to_retrieve:
                neighbour_ids[:, chunk_idx, :, :] = self.retriever.retrieve_neighbours(input_ids[:, chunk_idx * self.retrieval_interval : chunk_idx * self.retrieval_interval + self.config.chunk_size])
