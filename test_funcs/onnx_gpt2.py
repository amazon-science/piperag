...
# The repo: https://github.com/onnx/models/tree/main/text/machine_comprehension/gpt-2 
# The script for export and inference: https://github.com/onnx/models/blob/main/text/machine_comprehension/gpt-2/dependencies/GPT2-export.py
...
def save_model(name, model, inputs, outputs, input_names=None, output_names=None, dynamic_axes=None, **kwargs):
    ...
    # print(kwargs.items())
    # dynamic_axes=None
    print("inputs_flatten:", inputs_flatten)
    print("outputs_flatten:", outputs_flatten)

    print("Inputs:", inputs)
    print("Input names:", input_names)
    print("Output names:", output_names, flush=True)
    ...
    if isinstance(model, torch.jit.ScriptModule):
        ...
        # torch.onnx.export(model, inputs, model_dir, verbose=True, input_names=input_names,
        #                   output_names=output_names, dynamic_axes=dynamic_axes, **kwargs)
        #                 #   output_names=output_names, example_outputs=outputs, **kwargs)
        # wenqi's tuple arg input
        torch.onnx.export(model, ({"input_ids": inputs}), model_dir, verbose=True, input_names=input_names,
                          output_names=output_names, dynamic_axes=dynamic_axes, **kwargs)
    ...
    save_data(test_data_dir, "output", output_names, [outputs_flatten[0][0]])

    # (Pdb) outputs_flatten[0]
    #           [-3.5224e-01, -2.2143e+00,  1.8432e+00,  ..., -1.2932e+00,
    #         2.3036e-02, -1.3826e-01],
    #       [-1.7767e+00, -1.3178e+00,  1.4964e+00,  ..., -1.1139e+00,
    #         2.0167e+00, -6.4063e-01],
    #       [-1.6855e+00, -1.8139e+00,  1.8516e+00,  ..., -1.6275e+00,
    #         1.0612e+00, -1.4730e+00]]]]))), hidden_states=None, attentions=None, cross_attentions=None)
    # outputs_flatten[0]['past_key_values']
    # (Pdb) save_data(test_data_dir, "output", output_names, [outputs_flatten[0][0]])
...
def gpt2_test():
    ...
    for model_class, tokenizer_class, pretrained_weights, save_name in MODELS:
        ...
        with torch.no_grad():
            ...
            # output_2 = [model(input_ids_2)[0][0]]
